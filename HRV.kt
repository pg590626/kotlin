/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
/* elabora il file dello Stress estratto da Notify e ricava il valore    */
/* medio per ogni giorno da mezzanotte alle 6                            */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */

import java.io.File

data class HRV(val stress: Int, val data: String, val ora: String)
...
fun main() {
    ...
    val filename = "\\D:\\Dati\\Allenamenti\\Salute\\StressAF.csv"
    val fileInp = File(filename)
    // nome dei file di output
    val (nam, ext) = filename.split(".")
    val fileOut = File("$nam.Mod.$ext")
    // liste dei valori
    val tmp1 = mutableListOf<HRV>()            // lista di TUTTE le misure HRV
    ...
    if (fileInp.exists()) {
        ...
        fileInp.forEachLine {
            ...
            if (tmp2[0] != "Stress") {
                ...
                val tmp3 = HRV(tmp2[0].toInt(), tmp2[2], tmp2[3])
    ...
    for (day in days) {
        ...
        val giorno3 = giorno2.map {it.stress}
